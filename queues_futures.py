...
in some concurrent programming languages. They describe an object that 
acts as a proxy for a result that is initially unknown,
usually because the computaton of its value is not yet complete.
"""
...
def queued():
    que = Queue()
    t = Thread(target=test_que, args=['Mario', que])
    t.start()
...
def pooled():
    ...
    with ThreadPoolExecutor(max_workers=workers) as ex:
        for x in range(workers):
            ...
            future = ex. submit(test_future, 'Mario ' + str(x))
            ret.append(future)
    logging.info('Do something on the main thread')
    for r in ret:
